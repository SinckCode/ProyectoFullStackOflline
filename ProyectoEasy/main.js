...
async function processStack() {
    let stack = JSON.parse(localStorage.getItem('offlineRequests')) || [];
    let updatedStack = []; // para almacenar las solicitudes que no pudieron procesarse
    ...
    while (stack.length > 0) {
        ...
        try {
            ...
            updatedStack.push(request); // añadir la solicitud que falló a la pila actualizada
        }
    }

    localStorage.setItem('offlineRequests', JSON.stringify(updatedStack)); // actualizar la pila en localStorage
}

let isProcessing = false; // Variable para rastrear si la función processStack está en ejecución
    ...
    setInterval(async () => {
        try {
            ...
            if (response.ok) {
                ...
                if (!isProcessing) { // Verificar si la función processStack está actualmente en ejecución
                    isProcessing = true; // Marcar que la función está en ejecución
                    await processStack(); // Procesar las solicitudes en la pila
                    isProcessing = false; // Marcar que la función ha terminado de ejecutarse
                }
            } else {
